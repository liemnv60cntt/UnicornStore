...
<div class="row mb-5 mx-auto p-2" style="max-width: 1200px;">
    <?php
    $_cate = $cate->get_cate_byID($cateID);
    if ($_cate) {
        $result_cate = $_cate->fetch_assoc();
        echo "<h5>
    ...
    <div class="bg-white pb-3 pt-1 my-2 rounded-3 shadow-sm">
        ...
        <div class="container" id="myBtnContainer">
            ...
            <?php
            $product_by_cate = $cate->get_distinct_product_brandName_by_cate($cateID);
            if ($product_by_cate) {
                while ($result_search = $product_by_cate->fetch_assoc()) {

            ?>
                    <button class="btn btn-sm btn-outline-secondary mt-1" onclick="filterSelection('<?php echo $result_search['brandName'] ?>')"> <?php echo $result_search['brandName'] ?></button>
            <?php
                }
            }
            ?>
    ...
    <!-- Bộ lọc -->
    <?php
    $product_by_cate = $cate->get_product_by_cate($cateID);
    if ($product_by_cate) {
        while ($result_search = $product_by_cate->fetch_assoc()) {
...
<div>
    <ul class="pagination justify-content-center mt-4">
        <?php
        $rowsPerPage = 4;
        $product_all = $cate->get_all_product_by_cate($cateID);
        if($product_all){
            $numRows = mysqli_num_rows($product_all);
            $maxPage = ceil($numRows / $rowsPerPage);
            // Nút back
            if ($_GET['page'] > 1) {
                echo '<li class="page-item"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . "?cateID=$cateID&page=" . ($_GET['page'] - 1) . '"><i class="fas fa-chevron-left"></i></a></li>';
            } else {
                echo '<li class="page-item disabled"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . "?cateID=$cateID&page=" . ($_GET['page'] - 1) . '"><i class="fas fa-chevron-left"></i></a></li>';
            }
            // Nút phân trang
            for ($i = 1; $i <= $maxPage; $i++) {
                if ($i == $_GET['page']) {
                    ...
                    echo '<li class="page-item"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . "?cateID=$cateID&page=" . $i . '">' . $i . '</a></li>';
            }
            // Nút next
            if ($_GET['page'] < $maxPage) {
                echo '<li class="page-item"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . "?cateID=$cateID&page=" . ($_GET['page'] + 1) . '"><i class="fas fa-chevron-right"></i></a></li>';
            } else {
                echo '<li class="page-item disabled"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . "?cateID=$cateID&page=" . ($_GET['page'] + 1) . '"><i class="fas fa-chevron-right"></i></a></li>';
            }
        }
